    ...
    if (takeFromClipboard) {
        try {
            ...
            if (clipboardItems.length === 0) {
                ...
                const types = firstClipboardItem.types.filter(
                    (type) => type !== 'text/html',
                )
                let type = null
    ...
    if (insertFileModalType === 'url') {
        if (insertFileModalLinkLabel === '') {
            ...
            document.execCommand(
                'insertHTML',
                false,
                `<a href="${insertFileModalFile}" target="_blank" contenteditable="false">${insertFileModalLinkLabel}</a>`,
            )
        }
...
<Modal on:close-modal={() => (showInsertFileModal = false)}>
    <form on:submit|preventDefault={uploadFile}>
        ...
        <a
            href="#from-url"
            on:click|preventDefault={() => (insertFileModalType = 'url')}
            class:td-n={insertFileModalType === 'url'}>From URL</a
        >
        |
        <a
            href="#from-file"
            on:click|preventDefault={() => (insertFileModalType = 'file')}
            class:td-n={insertFileModalType === 'file'}>From File</a
        >
        {#if insertFileModalType === 'url'}
        ...
        {:else}
            <label class="d-b mt-1em"
                ...
                <input
                    type="file"
                    bind:files={insertFileModalFile}
                    required
                    class="w-100p"
                    use:focus
                    disabled={takeFromClipboard}
                />
            </label>
            <label
                class="mt-1em"
            ...
            >
                <div>
                    <input
                        type="checkbox"
                        style="margin-left: 0;"
                        bind:checked={takeFromClipboard}
                    />
                </div>
        ...
        <div style="width: 16rem; margin-top: 0.3rem">
            If link label is provided, inserted file will be inserted as a link
            instead of a image.
        </div>
